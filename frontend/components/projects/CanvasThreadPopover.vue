...
function getCommentColor(comment: CommentThread['comments'][0]) {
    ...
    // Try author_id first, then guest_name
...
<template>
    <div class="absolute z-[100]" :style="{
        left: `${x * 100}%`,
        top: `${y * 100}%`,
        ...
        <div class="relative">
            <div class="absolute left-1/2 bottom-0 -translate-x-1/2 pb-1">
                <CommentIcon :initial="pinInitial" size="lg" :color="pinColor" />
            </div>

            <div
                class="absolute left-6 top-0 w-[320px] max-w-[90vw] rounded-xl bg-white shadow-xl border border-gray-100 overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
                <div class="flex items-center justify-between border-b p-3 bg-gray-50/50">
                    ...
                    <div class="flex items-center gap-2">
                        <button v-if="!thread.is_resolved && canResolveThreads" @click="$emit('resolve')"
                ...
                <div class="flex flex-col max-h-[300px] overflow-y-auto">
                    <div v-for="(comment, index) in thread.comments" :key="comment.id" class="p-4"
                        :class="{ 'border-t': index > 0 }">
                        <div class="flex items-center gap-2 mb-1">
                            <div class="h-6 w-6 rounded-full flex items-center justify-center text-xs font-bold text-cs-charcoal"
                        ...
                        <p class="text-sm text-gray-700 pl-8 leading-relaxed">{{ comment.content }}</p>
                    </div>
                </div>

                <div class="p-3 bg-gray-50 border-t">
                    <div class="flex items-center gap-2">
                        <div class="h-6 w-6 rounded-full flex items-center justify-center text-xs font-bold text-cs-charcoal shrink-0"
                        ...
                        <div
                            ...
                            <input v-model="replyContent" type="text" placeholder="Reply"
                            ...
                            <button
                                ...
                                :disabled="!replyContent.trim()" @click="handleReply">
