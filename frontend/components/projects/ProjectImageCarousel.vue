    ...
    <div class="w-full">
        ...
        <div v-else class="space-y-3">
            ...
            <div ref="contentRef"
                ...
                <div class="flex items-center justify-center transition-transform duration-75 ease-out" :style="{
                    transform: `translate(${translate.x}px, ${translate.y}px) scale(${zoom})`,
                }">
                    <img v-if="activePhoto?.url" ref="imageRef" :src="activePhoto.url"
                        :alt="activePhoto?.title || activePhoto?.filename || 'Project photo'"
                        class="max-h-[60vh] max-w-full object-contain" loading="lazy">
                    <div v-else class="text-sm text-cs-whiteish/80">
                        Photo not available.
                    </div>
                </div>

                <div class="absolute inset-y-0 left-0 flex items-center ml-2">
                    <Button variant="regular" size="icon" class="h-8 w-8 rounded-lg" @click.stop="goPrevious"
                        @pointerdown.stop>
                ...
                <div class="absolute inset-y-0 right-0 flex items-center mr-2">
                    <Button variant="regular" size="icon" class="h-8 w-8 rounded-lg" @click.stop="goNext"
                        @pointerdown.stop>
            ...
            <div class="flex flex-wrap items-center justify-center gap-2">
                <button v-for="(photo, index) in props.photos" :key="photo.id || index" type="button"
                    class="h-2 w-2 rounded-full border border-white/50 transition"
                    :class="index === activeIndex ? 'bg-white' : 'bg-transparent opacity-60'" @click="goTo(index)" />
            </div>
        </div>
    </div>
</template>
