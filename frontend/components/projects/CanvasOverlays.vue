...
<template>
  ...
  >
    ...
    >
      ...
      >
        <CommentPin
          :label="pin.data.initial"
          :comment="pin.data.comment"
          :author-name="pin.data.authorName"
          :color="pin.data.color"
          :is-expanded="false"
          @mouseenter="emit('pinHover', pin.id)"
          @mouseleave="emit('pinLeave', pin.id)"
          @click.stop="emit('pinClick', pin.id)"
        />
      </div>
    </template>
    ...
    <CanvasCommentInput
      ...
      @cancel="emit('cancelComment')"
    />

    <CanvasThreadPopover
      v-if="activeThread"
      :thread="activeThread"
      :x="activeThread.pin_x"
      :y="activeThread.pin_y"
      :zoom="zoom"
      :current-user-initial="currentUserInitial"
      :can-resolve-threads="canResolveThreads"
      :author-color-map="authorColorMap"
      :pin-color="pins.find(p => p.id === activeThread?.id)?.data.color"
      class="pointer-events-auto"
      @pointerdown.stop
      @reply="(content) => emit('reply', content)"
      @resolve="emit('resolveThread', activeThread!)"
      @close="emit('closeThread')"
    />
  </div>
</template>
